package core.xmate.common;

import core.xmate.common.task.AbsTask;

/**
 * Created by wyouflf on 15/6/11.
 * 任务管理接口
 */
public interface TaskController {

    /**
     * 在UI线程执行runnable.
     * 如果已在UI线程, 则直接执行.
     *
     * @param runnable
     */
    void autoPost(Runnable runnable);

    /**
     * 在UI线程执行runnable.
     * post到msg queue.
     *
     * @param runnable
     */
    void post(Runnable runnable);

    /**
     * 在UI线程执行runnable.
     *
     * @param runnable
     * @param delayMillis 延迟时间(单位毫秒)
     */
    void postDelayed(Runnable runnable, long delayMillis);

    /**
     * 在后台线程执行runnable
     *
     * @param runnable
     */
    void run(Runnable runnable);

    /**
     * 移除post或postDelayed提交的, 未执行的runnable
     *
     * @param runnable
     */
    void removeCallbacks(Runnable runnable);

    /**
     * 开始一个异步任务
     *
     * @param task
     * @param <T>
     * @return
     */
    <E, T> AbsTask<E, T> start(AbsTask<E, T> task);

    /**
     * @param task
     * @param e
     * @param <E>
     * @param <T>
     * @return
     */
    <E, T> AbsTask<E, T> start(AbsTask<E, T> task, E e);

    /**
     * 同步执行一个任务
     *
     * @param task
     * @param <T>
     * @return
     * @throws Throwable
     */
    <E, T> T startSync(AbsTask<E, T> task) throws Throwable;

    /**
     * @param task
     * @param e
     * @param <E>
     * @param <T>
     * @return
     * @throws Throwable
     */
    <E, T> T startSync(AbsTask<E, T> task, E e) throws Throwable;
}
