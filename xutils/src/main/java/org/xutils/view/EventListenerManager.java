	...
	public static void addEventMethod (
		...
		try {
			...
			if (view != null) {
				...
				if (methodName == null || "".equals(methodName)) {//method为空
					Method[] declaredMethods = listenerType.getDeclaredMethods();
					if (declaredMethods != null && declaredMethods.length == 1) {//指定的监听器接口之中有且只有一个回调方法
						methodName = declaredMethods[0].getName();
					} else {
						throw new IllegalArgumentException("监听器回调方法不存在或者不唯一");
					}
				}
